...
export default class NoticeDetail extends Component {
    ...
    renderTopOpenAttachmentBtn() {
        if (this.state.dataArray.length) {
            return (<TouchableOpacity onPress={this.showFujianList.bind(this)}>
                <View style={styleSheet.topbg}>
                    <Text style={styleSheet.openAddText} >{'查看附件'}</Text>
                </View>
            </TouchableOpacity>)
        } else {
            return null;
    ...
    render() {
        ...
        for (var i = 0; i < this.state.dataArray.length; i++) {
            attachments.push(
                <Button key={i} onPress={this.openFujian.bind(this, i)} title={'打开附件' + (i + 1)} style={styleSheet.openButton}></Button>
            );
        }
        var options = [];
        for (var i = 0; i < this.state.dataArray.length; i++) {
        ...
        return (
            <View style={styleSheet.bottomView}>
                ...
                <ScrollView contentContainerStyle={styleSheet.contentContainer}>
                    ...
                    <Text style={styleSheet.txtTitle}> {this.state.title} </Text>
                    <WebView
                        automaticallyAdjustContentInsets={false}
                        style={styleSheet.txtContent}
